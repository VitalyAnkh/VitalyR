...
use model::info::{OuisrcInfo,OuisrcInfoMsgs};
use router::ConnDsl;


impl Handler<OuisrcInfo> for ConnDsl {
    type Result = Result<OuisrcInfoMsgs, Error>;

    fn handle(&mut self, ouisrc_info: OuisrcInfo, _: &mut Self::Context) -> Self::Result {
        ...
        let mut ouisrc_info:Vec<u32> = vec![];
        ouisrc_info.push(user_result.len() as u32);
        ouisrc_info.push(themes_result.len() as u32);
        ouisrc_info.push(blog_result.len() as u32);
        ouisrc_info.push(comments_result.len() as u32);
        ouisrc_info.push(saves_result.len() as u32);
        Ok(OuisrcInfoMsgs{
                status: 200,
                message : "get  user_id info success.".to_string(),
                ouisrc_info: ouisrc_info,
        })
    }
}